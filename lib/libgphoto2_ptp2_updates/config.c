...
/*		LOG_ON_PTP_E (ptp_canon_eos_requestdevicepropvalue (params, PTP_DPC_CANON_EOS_LensAdjustParams)); */
	...
	if (is_canon_eos_m (params)) {
		...
		//ct_val.u16 = 0x0008;
		//ct_val.u16 = 0x0008;
		//C_PTP (ptp_canon_eos_setdevicepropvalue (params, PTP_DPC_CANON_EOS_EVFOutputDevice, &ct_val, PTP_DTC_UINT16));//PTP_DTC_UINT16));
...
//
    //                    //propval.u16 = 0x0001;
    //                    //C_PTP (ptp_setdevicepropvalue (params, 0xd38c, &propval, PTP_DTC_UINT16)); // PC mode?
	//		return GP_OK;
	//	}
	//	break;
...
static struct submenu camera_actions_menu[] = {
	...
	{ N_("Drive Panasonic Manual focus"),   "manualfocusdrive", 0,  PTP_VENDOR_PANASONIC, 0									,_get_Panasonic_MFDrive,       	_put_Panasonic_MFDrive },
...
_get_config (Camera *camera, const char *confname, CameraWidget **outwidget, CameraList *list, GPContext *context)
{
	...
	int hiddenPropertiesLen = 0;
	uint16_t hiddenProperties[5];
	if(params->deviceinfo.VendorExtensionID == PTP_VENDOR_FUJI)
	{
		hiddenPropertiesLen = 5;
	...
	for (i=0;i<params->deviceinfo.DevicePropertiesSupported_len + hiddenPropertiesLen;i++) {
		uint16_t		propid;
		if(i<params->deviceinfo.DevicePropertiesSupported_len)
		{
			propid = params->deviceinfo.DevicePropertiesSupported[i]; 
			if(!have_prop(camera,params->deviceinfo.VendorExtensionID,propid)) continue; // skip if not supported
		}
		else
		{
			propid = hiddenProperties[i - params->deviceinfo.DevicePropertiesSupported_len]; 
		}
		char			buf[20], *label;
		PTPDevicePropDesc	dpd;
