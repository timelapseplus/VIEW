...
int main(int argc, char *argv[])
{
    ...
    if(!invalidArgs)
    {
        ...
        if(found[D])
        {
            uint8_t auxReading, syncReading, ringReading, i = 0, sh = 0;
            _bulb_init();
            bulb_set_shutter(0);
            for(;;)
            {
                auxReading = bulb_read_aux();
                ringReading = bulb_read_aux_ring();
                syncReading = bulb_read_sync();
                printf("Shutter: %d, AUX: %d, RING: %d, SYNC: %d\n", sh, auxReading, ringReading, syncReading);
                i++;
                if(i == 4)
                {
                    sh = 1;
