...
//int64_t timingConstant = 1500; // 600 baud
//int64_t timingConstant = 773; // 1200 baud (works)
//int64_t timingConstant = 525; // 1800 baud
int64_t timingConstant = 399; // 2400 baud
...
uint8_t aux2out_serial_in(char **s, unsigned int timeout_ms)
{
    ...
    for(;;) {
        ...
        for(;;) // wait for start bit
        {
            ...
            int64_t diff = _microSecondDiff(&now, &startTime);
            if(AUX_TIP_IN()) break;
            if(diff >= timingConstant + (timingConstant / 2) && bytes > 0 || diff > timeout_us) {
                done = 1;
                break;
            }
        }
        ...
        for(;;) // pause for 1.5 bits
        {
            ...
            gettimeofday(&now, 0);
            int64_t diff = _microSecondDiff(&now, &startTime);
            if(diff >= timingConstant + (timingConstant / 2)) break;
        }
        buf[bytes] = '\0';
...
uint8_t aux2out_serial_transaction(char *send, char **received, unsigned int timeout_ms) {
    aux2out_init();
    uint8_t n = aux2out_serial_out(send);
    //printf("AUX2: sent %d bytes: '%s'\n", n, send);
    n = aux2out_serial_in(received, timeout_ms);
    //printf("AUX2: received %d bytes: '%s'\n", n, *received);
    return n;
}
...
int main() {
    printf("starting...\n");
    //uint8_t n = aux2out_serial_out("G300M2V2000\r\n");
    char *rec;
    uint8_t n = aux2out_serial_transaction("G500\n", &rec, 1000);
