...
void GPhoto2::Async_List(uv_work_t *req) {
  ...
  gphoto->connections += autodetect(
    list_req->list, list_req->context, &portInfoList, &abilitiesList);
  gphoto->setAbilitiesList(abilitiesList);
  gphoto->setPortInfoList(portInfoList);
  if (gphoto->connections == 0) {
    gphoto->close();
  }
}
...
void GPhoto2::close() {
  ...
  gp_port_info_list_free(this->portinfolist_);
  gp_camera_free(this->_camera);
  gp_context_unref(this->context_);
}

int GPhoto2::decrementConnections() {
  this->connections = std::max(0, this->connections - 1);
