...
int GPCamera::setWidgetValue(set_config_request *req) {
  ...
  if(!config_) {
    ret = gp_camera_get_config(req->camera, &config_, req->context);
    if (ret < GP_OK) return ret;
  }

  ret = gp_widget_get_child_by_name(config_, req->key.c_str(), &child);
  if (ret < GP_OK) {
    //gp_widget_free(config_);
    return ret;
  }
  ...
  ret = gp_camera_set_config(req->camera, config_, req->context);
  //gp_widget_free(config_);
  if (ret < GP_OK) return ret;
  return ret;
}
