...
struct timespec last_get_config_time, now;
...
int GPCamera::setWidgetValue(set_config_request *req) {
  ...
  if(configRead) {
    ...
    time_since_last = (now.tv_sec - last_get_config_time.tv_sec);
    time_since_last += (now.tv_nsec - last_get_config_time.tv_nsec) / 1000000000.0;

    if(time_since_last > 1.0) configRead = 0;
  }

  clock_gettime(CLOCK_MONOTONIC, &last_get_config_time);
...
void GPCamera::downloadPicture(take_picture_request *req) {
  ...
  printf("GPHOTO2: retrieving file '%s' from folder '%s'\n", name.c_str(), folder.str().c_str());
  ...
  if (retval == GP_OK && !req->keepOnCamera) {
    ...
    if(retval != GP_OK) {
      printf("GPHOTO2: Error removing image from camera (%d).\n", retval);
    }
    retval = GP_OK;
  }
