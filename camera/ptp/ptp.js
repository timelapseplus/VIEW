            ...
            if (msg.type == 'event') {
                ...
                if (msg.event == 'connected') {
                    ...
                    if(worker.model != 'SonyWifi' && worker.model.match(/sony/i)) {
                        ...
                        if(worker.model.match(/(a6300|A7r II|A7r III|A7s II|7M2|A7 II|A7 III|ILCE-7M2|ILCE-7M2|A7s|a6500|a99 M2|a99 II|a77 II|a68|A9)/i)) {
...
ptp.on('unsupported', function(device) {
    ...
    var bus = device.busNumber.toString();
    var dev = device.deviceAddress.toString();
    while(bus && bus.length < 3) bus = '0' + bus;
    while(dev && dev.length < 3) dev = '0' + dev;
    var port = 'usb:' + bus + ',' + dev;
...
camera.unmountSd = function(callback) {
    if (camera.sdMounted) {
        ...
        try {
            ...
            console.log("error attempting unmounting: ", e);
            sdUnmountErrors++;
            if(sdUnmountErrors < 3) {
                return setTimeout(function(){
                    camera.unmountSd(callback);
